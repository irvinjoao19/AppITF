...
public class TablePerfilAdapter extends AbstractTableAdapter<ColumnHeaderModel, RowHeaderModel,
        CellModel> {

    private TablePerfilViewModel tablePerfilViewModel;
    private OnItemClickListener.ProgramacionPerfilListener listener;

    public TablePerfilAdapter(Context p_jContext, OnItemClickListener.ProgramacionPerfilListener listener) {
        super(p_jContext);
        this.tablePerfilViewModel = new TablePerfilViewModel();
    ...
    public void onBindCellViewHolder(AbstractViewHolder holder, Object p_jValue, int
        ...
        if (holder instanceof CellViewHolder) {
            ...
            ((CellViewHolder) holder).setCellModel(cell, p_nXPosition, listener);
        }
    }
    ...
    public int getCellItemViewType(int position) {
        return tablePerfilViewModel.getCellItemViewType(position);
    }
     ...
     */
    public void setUserList(List<ProgramacionPerfil> userList) {
        // Generate the lists that are used to TableViewAdapter
        tablePerfilViewModel.generateListForTableView(userList);

        // Now we got what we need to show on TableView.
        setAllItems(tablePerfilViewModel.getColumHeaderModeList(), tablePerfilViewModel
                .getRowHeaderModelList(), tablePerfilViewModel.getCellModelList());
    }
}