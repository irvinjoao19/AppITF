...
import kotlinx.android.synthetic.main.fragment_reporte_mes.*
import javax.inject.Inject

private const val ARG_PARAM1 = "param1"
private const val ARG_PARAM2 = "param2"

class ReporteMesFragment : DaggerFragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_reporte_mes, container, false)
    }
            ...
            if (it.isNotEmpty()) {
                ...
                for (i in it.indices) {
                    if (i == 0) {
                        ...
                        if (f1 != null) {
                            halfGauge1.value = f1.porcentajeMes.toDouble()
                            getData(f1, tabla1)
                        }
                    } else {
                        val f2: RptGeneral? = it[i]
                        if (f2 != null) {
                            halfGauge2.value = f2.porcentajeMes.toDouble()
                            getData(f2, tabla2)
                        }
                    }
                }
    ...
    private fun getData(row: RptGeneral, table: TableLayout) {
        ...
        tv3.text = row.porcentajeMes.toString()
        ...
        tv4.text = row.saldoMes.toString()
