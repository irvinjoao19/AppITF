...
import kotlinx.android.synthetic.main.fragment_reporte_sub_fecha.*
...
class ReporteSubFechaFragment : DaggerFragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_reporte_sub_fecha, container, false)
    }
    ...
    private fun bindUI() {
        ...
        itfViewModel.getRptGeneralFecha().observe(viewLifecycleOwner, {
            if (it.isNotEmpty()) {
                val count: Int = tablaFecha.childCount
                for (i in 2 until count) {
                    val child: View = tablaFecha.getChildAt(i)
    ...
    private fun getData(data: List<RptGeneral>) {
        ...
        for (i in 0 until rows) {
            ...
            tv3.text = row.cuotaFecha.toString()
            ...
            tv4.text = row.numeroVisita.toString()
            ...
            tv5.text = row.porcentajeFecha.toString()
            ...
            tr.addView(tv4)
            tr.addView(tv5)
            tablaFecha.addView(tr, trParams)
        }
    }
